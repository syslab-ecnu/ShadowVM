...
void run_SSB_Q21_date(ShadowVMClient & client) {
	run_load_date(client);
	PipelineTest query("q21_date");
...
void run_SSB_Q21_part(ShadowVMClient & client) {
	run_load_part(client);
	PipelineTest query("q21_part");
...
void run_SSB_Q21_supplier(ShadowVMClient & client) {
	run_load_supplier(client);
	PipelineTest query("q21_supplier");
...
void run_SSB_Q21(ShadowVMClient & client) {
	run_SSB_Q21_date(client);
	run_SSB_Q21_part(client);
	run_SSB_Q21_supplier(client);
	run_load_lineorder(client);

	PipelineTest query("q21_rc");
	query.pipe_info_.add_pre_pipeline_id("lineorder");
	query.pipe_info_.add_pre_pipeline_id("q21_date");
	query.pipe_info_.add_pre_pipeline_id("q21_part");
	query.pipe_info_.add_pre_pipeline_id("q21_supplier");
   ...
   query.AddOperator(OperatorInfo::ReduceBy, {{"dim","$(1.1), $(2.1)"}, {"metric","$(0.12)/100"}}, result);
	...
	Object * rc = ShadowVMService::cached_obj["q21_rc"];
 	rc->Print();
