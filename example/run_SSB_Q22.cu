...
void run_SSB_Q22_date(ShadowVMClient & client) {
	run_load_date(client);
	PipelineTest query("q22_date");
...
void run_SSB_Q22_part(ShadowVMClient & client) {
	run_load_part(client);
	PipelineTest query("q22_part");
...
void run_SSB_Q22_supplier(ShadowVMClient & client) {
	run_load_supplier(client);
	PipelineTest query("q22_supplier");
...
void run_SSB_Q22(ShadowVMClient & client) {
	run_SSB_Q22_date(client);
	run_SSB_Q22_part(client);
	run_SSB_Q22_supplier(client);
	run_load_lineorder(client);

	PipelineTest query("q22_rc");
	query.pipe_info_.add_pre_pipeline_id("lineorder");
	query.pipe_info_.add_pre_pipeline_id("q22_date");
	query.pipe_info_.add_pre_pipeline_id("q22_part");
	query.pipe_info_.add_pre_pipeline_id("q22_supplier");
	...
	Object * rc = ShadowVMService::cached_obj["q22_rc"];
 	rc->Print();
