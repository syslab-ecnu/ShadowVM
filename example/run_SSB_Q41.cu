...
void run_SSB_Q41_date(ShadowVMClient & client) {
	run_load_date(client);
	PipelineTest query("q41_date");
...
void run_SSB_Q41_part(ShadowVMClient & client) {
	run_load_part(client);
	PipelineTest query("q41_part");
...
void run_SSB_Q41_supplier(ShadowVMClient & client) {
	run_load_supplier(client);
	PipelineTest query("q41_supplier");
...
void run_SSB_Q41_customer(ShadowVMClient & client) {
	run_load_customer(client);
	PipelineTest query("q41_customer");
...
void run_SSB_Q41(ShadowVMClient & client) {
	run_SSB_Q41_date(client);
	run_SSB_Q41_part(client);
	run_SSB_Q41_supplier(client);
	run_SSB_Q41_customer(client);

	run_load_lineorder(client);

	PipelineTest query("q41_rc");
	query.pipe_info_.add_pre_pipeline_id("lineorder");
	query.pipe_info_.add_pre_pipeline_id("q41_date");
	query.pipe_info_.add_pre_pipeline_id("q41_part");
	query.pipe_info_.add_pre_pipeline_id("q41_supplier");
	query.pipe_info_.add_pre_pipeline_id("q41_customer");
	...
	Object * rc = ShadowVMService::cached_obj["q41_rc"];
 	rc->Print();
