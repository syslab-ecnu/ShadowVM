...
pair<ObjectInfo, Object*> query_date_Q43() {
	auto date = load_date();
	...
	return make_pair(schema, query.Execute(date.second, {},true, true));
}

pair<ObjectInfo, Object*> query_part_Q43() {
	auto part = load_part();
	//part.second->Print();
...
pair<ObjectInfo, Object*> query_supplier_Q43() {
	auto supplier = load_supplier();
	ObjectInfo schema = CreateSchema({TInt32, TString, TString}, {4, 25, 40});
...
pair<ObjectInfo, Object*> query_customer_Q43() {
...
void test_SSB_Q43(bool isGPU) {
	...
	auto date = query_date_Q43();   date.second->Print();
    auto part = query_part_Q43();    part.second->Print();
	auto supplier = query_supplier_Q43();  supplier.second->Print();
	auto customer = query_customer_Q43();  customer.second->Print();
//	return;
	...
	PipelineTest query("test_SSB_Q43");
